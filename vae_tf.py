...
# refer to https://github.com/y0ast/VAE-TensorFlow/blob/master/main.py

"""
    这版和Keras思路一样，只是加了l2正则化。并且最后测试是直接看loss。
"""
...
class MNISTIterator(object):
    def __init__(self, dataset):
        ...
        self.train = Iterator(x_train, y_train)
        self.test = Iterator(x_test, y_test)

mnist = MNISTIterator(mnist_data)
...
with tf.Session() as sess:
    summary_writer = tf.summary.FileWriter('tf_record',
                                          graph=sess.graph)
    if os.path.isfile("tf_model/model.ckpt"):
